...
function getCrossPointFromPoints(p1,p2,p3,p4){
    var c1 = getCross(subtractVector(p4,p3),subtractVector(p3,p1));
    var c2 = getCross(subtractVector(p4,p3),subtractVector(p2,p1));
    if (equalFloats(c1, 0) && equalFloats(c2, 0)){
        return p1;   
    }
    // 交点がない
    if(equalFloats(c2, 0)) return;
    var c = c1 / c2;
    var vec = multiplyVector(subtractVector(p2,p1),c);
    return (addVector(p1,vec));
}
...
for(var i = 0;i < q;++i){
    ...
    var point2 = new Point2D(nums[2],nums[3]);
    var convex = getConvexCut(points,point1,point2);
    console.log(getArea(convex).toFixed(8));
}